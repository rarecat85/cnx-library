...
| 1.1.4 | 회사 이메일 검증 | 외부 이메일 입력 (예: "test@gmail.com") | "@concentrix.com 이메일만 사용 가능" 오류 | | |
| 1.1.5 | 비밀번호 확인 불일치 | 비밀번호와 확인이 다르게 입력 | 불일치 오류 메시지 | | |
| 1.1.6 | 정상 회원가입 | 모든 정보 올바르게 입력 후 가입 | 인증 이메일 발송 안내 표시 | | |
| 1.1.7 | 중복 이메일 가입 시도 | 이미 가입된 이메일로 재가입 | "이미 사용 중인 이메일" 오류 | | |

**테스트 데이터:**
\`\`\`
이메일: test.user@concentrix.com
비밀번호: Test1234!
이름: 테스트유저
근무지: 용산
\`\`\`
...
| # | 테스트 항목 | 테스트 방법 | 예상 결과 | 결과 | 비고 |
|---|------------|------------|----------|------|------|
| 3.2.1 | 도서 선택 | 대여 가능한 도서 카드 클릭 | 체크박스 선택됨 | | |
| 3.2.2 | 복수 도서 선택 대여 | 여러 권 선택 후 "대여 신청" 클릭 | 선택한 모든 도서 대여 처리 | | |
| 3.2.3 | 개별 도서 대여 | 도서 카드의 "대여" 버튼 클릭 | 해당 도서 즉시 대여 처리 | | |
| 3.2.4 | 같은 센터 도서 대여 | 내 근무지와 동일 센터 도서 대여 | 즉시 대여 완료 | | |
| 3.2.5 | 대여 후 상태 변경 | 대여 완료 후 도서 카드 확인 | "대여중" 상태로 변경 | | |
| 3.2.6 | 다른 센터 도서 대여 신청 | 다른 센터 도서의 "대여 신청" 클릭 | "대여 신청 중" 상태로 변경, 관리자 승인 대기 | | |
| 3.2.7 | 대여중인 도서 | 대여중인 도서 카드 확인 (타인/본인 무관) | 대여 버튼 미노출, "대여중" 상태 표시, 선택 불가 | | |
| 3.2.8 | 최대 대여 권수 제한 | 5권 대여 상태에서 추가 대여 시도 | "5권 대여중, 반납 후 대여 가능" 메시지 | | |
| 3.2.9 | 남은 대여 가능 권수 표시 | 3권 대여 상태에서 도서 목록 확인 | "2권 추가 가능" 표시 | | |
...
| 4.1.3 | 센터 필터링 | 센터 선택 드롭다운 변경 | 해당 센터 대여 도서만 표시 | | |
| 4.1.4 | 반납예정일 표시 | 대여 도서 카드 확인 | 반납예정일 표시 (대여일 +7일) | | |
| 4.1.5 | 도서 선택 | 도서 카드 클릭 | 체크박스 선택됨 | | |
| 4.1.6 | 단일 도서 반납 | 1권 선택 후 "반납하기" 클릭 | 반납 확인 → 반납 완료 | | |
| 4.1.7 | 복수 도서 반납 | 여러 권 선택 후 "반납하기" 클릭 | 선택한 모든 도서 반납 완료 | | |
...
| 4.3.3 | 읽은 책 삭제 | 읽은 책 선택 후 "삭제하기" 클릭 | 목록에서 제거 (대여 횟수 포함 전체 이력 삭제) | | |
...
| 4.4.5 | 비밀번호 변경 | "비밀번호 변경" 버튼 클릭 | 비밀번호 재설정 링크 이메일 발송 성공 메시지 | | |
...
| 5.2 | 센터 변경 | 센터 선택 드롭다운 변경 | 선택한 센터로 신청 대상 변경 | | |
| 5.3 | 도서 검색 | 검색창에 도서명 입력 후 검색 | 알라딘 API 검색 결과 표시 | | |
| 5.4 | 검색 결과 표시 | 검색 완료 후 확인 | 도서 목록 그리드 형태로 표시, 페이지네이션 | | |
| 5.5 | 베스트셀러 표시 | 베스트셀러 섹션 확인 | 알라딘 베스트셀러 스와이퍼 표시 | | |
| 5.6 | 이미 등록된 도서 | 등록된 도서 검색 | "이미 등록된 도서" 표시 | | |
| 5.7 | 도서 신청하기 | 미등록 도서의 "신청하기" 클릭 | 신청 확인 → 신청 완료 메시지 | | |
| 5.8 | 신청 후 상태 | 신청한 도서 재검색 | "신청완료" 상태 표시 | | |
| 5.9 | 초성 검색 | 초성으로 검색 (예: "ㅎㄹㅍㅌ") | 초성에 해당하는 도서 검색 결과 표시 | | |
| 5.10 | 검색 결과 없음 안내 | 결과 없는 검색어 입력 (예: "zzzxxx123") | "검색 결과가 없습니다" 안내 메시지 표시 | | |
| 5.11 | 세트 상품 자동 제외 | 일반 도서명 검색 (예: "해리포터") | 세트/전집 상품이 결과에서 자동 제외됨 | | |
...
| 6.1.3 | 센터 변경 | 센터 선택 드롭다운 변경 | 선택한 센터에 도서 등록 가능 | | |
| 6.1.4 | 도서 검색 | 도서명으로 검색 | 알라딘 검색 결과 그리드 표시 | | |
| 6.1.5 | 베스트셀러 표시 | 베스트셀러 섹션 확인 | 알라딘 베스트셀러 스와이퍼 표시 | | |
| 6.1.6 | 도서 등록 | 검색된 도서의 "등록하기" 클릭 | 등록 완료 메시지 | | |
| 6.1.7 | 중복 등록 방지 | 이미 등록된 도서 등록 시도 | "이미 등록된 도서" 메시지 | | |
| 6.1.8 | 도서 신청 목록 | 도서 신청 목록 섹션 확인 | 사용자들이 신청한 도서 표시 | | |
| 6.1.9 | 신청자 정보 표시 | 신청 도서 카드 확인 | 신청자 근무지, 이름 표시 | | |
| 6.1.10 | 신청 도서 등록 | 신청 도서의 "등록하기" 클릭 | 등록 완료, 신청자에게 알림 발송 | | |
...
| 6.2.2 | 센터 변경 | 센터 선택 드롭다운 변경 | 선택한 센터 도서만 표시 (모든 센터 관리 가능) | | |
| 6.2.3 | 등록 도서 목록 | 등록된 도서 목록 섹션 확인 | 센터별 등록 도서 표시 | | |
| 6.2.4 | 도서 검색 | 검색창에 도서명 입력 | 검색된 도서만 필터링 | | |
| 6.2.5 | 도서 삭제 | 도서 카드의 삭제 버튼 클릭 | 삭제 확인 → 삭제 완료 | | |
| 6.2.6 | 대여중 도서 삭제 시도 | 대여중인 도서 삭제 시도 | 삭제 불가 메시지 | | |
| 6.2.7 | 대여 현황 표시 | 대여중인 도서 카드 확인 | 대여자 정보 (근무지, 이름, 이메일) 표시 | | |
| 6.2.8 | 반납예정일 표시 | 대여중 도서 확인 | 반납예정일 표시 | | |
| 6.2.9 | 관리자 반납 처리 | "반납 처리" 버튼 클릭 | 반납 완료, 상태 변경 | | |
| 6.2.10 | 대여 신청 승인 | 대여 신청 도서의 "승인" 클릭 | 대여 처리 완료 | | |
