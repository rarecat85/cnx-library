  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        title: '2.2 신규 도서 (NEW)',
        items: [
          { id: '2.2.1', name: '신규 도서 스와이퍼', method: '센터별 신규 도서 섹션 확인', expected: '구매칸 도서만 표시 (NEW 배지)' },
          { id: '2.2.2', name: '구매칸 도서 NEW 표시', method: '위치가 "구매칸"인 도서 확인', expected: 'NEW 배지 표시' },
          { id: '2.2.3', name: '다른 위치 도서 NEW 미표시', method: '위치가 "1번 칸" 등인 도서 확인', expected: 'NEW 배지 미표시' },
          { id: '2.2.4', name: '위치 변경 시 NEW 제거', method: '구매칸 도서를 다른 위치로 변경 후 확인', expected: '신규 도서 섹션에서 사라짐' }
        ]
      }
    ]
  },
  {
    ...
    subsections: [
      ...
      },
      {
        title: '3.3 대여 다이얼로그',
        items: [
          { id: '3.3.1', name: '단일 도서 대여 다이얼로그', method: '"대여하기" 버튼 클릭', expected: '도서 정보 + 라벨번호 + 위치 카드 형태로 표시' },
          { id: '3.3.2', name: '라벨번호 선택', method: '같은 도서 여러 권일 때 대여 시도', expected: '라벨번호 선택 옵션 표시' },
          { id: '3.3.3', name: '위치 정보 확인', method: '대여 확인 다이얼로그에서 위치 확인', expected: '라벨번호와 위치 정보 표시' },
          { id: '3.3.4', name: '다권 대여 다이얼로그', method: '여러 도서 선택 후 "대여 신청" 클릭', expected: '각 도서별 표지, 제목, 저자, 라벨번호, 위치 표시' },
          { id: '3.3.5', name: '안내 메시지', method: '대여 확인 다이얼로그 하단 확인', expected: '"위치와 라벨번호를 확인 후 해당 도서를 가져가주세요" 표시' }
        ]
      }
    ]
  },
  {
    ...
    subsections: [
      ...
      },
      {
        title: '4.2 반납 다이얼로그',
        items: [
          { id: '4.2.1', name: '반납 확인 다이얼로그 스타일', method: '도서 선택 후 "반납하기" 클릭', expected: '대여 확인과 동일한 카드 스타일' },
          { id: '4.2.2', name: '도서 정보 표시', method: '반납 확인 다이얼로그 내용 확인', expected: '표지, 제목, 저자, 출판사, 라벨번호, 위치 표시' },
          { id: '4.2.3', name: '반납 안내 메시지', method: '반납 확인 다이얼로그 하단 확인', expected: '"위치와 라벨번호를 확인 후 해당 위치에 도서를 반납해주세요" 표시' },
          { id: '4.2.4', name: '다권 반납 스크롤', method: '여러 도서 선택 후 반납', expected: '도서 목록 세로 스크롤 가능' }
        ]
      },
      {
        title: '4.3 신청한 책 목록',
        items: [
          { id: '4.3.1', name: '신청한 책 섹션 표시', method: '도서 신청 후 마이페이지 확인', expected: '리스트로 신청 도서 표시 (10개씩 페이지네이션)' },
          { id: '4.3.2', name: '신청일 및 취소 버튼 표시', method: '신청 도서 카드 확인', expected: '신청일 표시, "신청 취소" 버튼 표시' },
          { id: '4.3.3', name: '신청 취소', method: '"신청 취소" 버튼 클릭', expected: '취소 확인 → 목록에서 제거' }
        ]
      },
      {
        title: '4.4 읽은 책 목록',
        items: [
          { id: '4.4.1', name: '읽은 책 목록 표시', method: '반납 완료 후 확인', expected: '반납한 도서가 읽은 책 목록에 추가' },
          { id: '4.4.2', name: '중복 대여 기록', method: '같은 책 재대여 후 반납', expected: '대여 횟수 증가, 목록에 1개만 표시' },
          { id: '4.4.3', name: '읽은 책 삭제', method: '읽은 책 선택 후 "삭제하기" 클릭', expected: '목록에서 제거' }
        ]
      },
      {
        title: '4.5 정보 수정',
        items: [
          { id: '4.5.1', name: '정보수정 페이지 접근', method: '"정보수정" 버튼 클릭', expected: '/mypage/edit 페이지로 이동' },
          { id: '4.5.2', name: '현재 정보 표시', method: '페이지 로딩 확인', expected: '현재 이름, 근무지 표시' },
          { id: '4.5.3', name: '이름 변경', method: '이름 수정 후 저장', expected: '변경 완료 메시지, 마이페이지 반영' },
          { id: '4.5.4', name: '근무지 변경', method: '근무지 선택 변경 후 저장', expected: '변경 완료, 센터 필터 기본값 변경' },
          { id: '4.5.5', name: '비밀번호 변경', method: '"비밀번호 변경" 버튼 클릭', expected: '비밀번호 재설정 링크 이메일 발송 성공 메시지' }
        ]
      },
      {
        title: '4.6 위치 안내 팝업',
        items: [
          { id: '4.6.1', name: '위치 아이콘 클릭', method: '대여 목록 도서의 위치 ⓘ 아이콘 클릭', expected: '위치 안내 팝업 표시' },
          { id: '4.6.2', name: '서가 이미지 표시', method: '위치 안내 팝업 내용 확인', expected: '해당 센터 서가 이미지와 위치 설명 표시' },
          { id: '4.6.3', name: '팝업 닫기', method: '팝업 외부 또는 닫기 버튼 클릭', expected: '팝업 정상 닫힘' },
          { id: '4.6.4', name: '위치 텍스트 클릭', method: '위치 텍스트 영역 클릭', expected: '팝업 열리지 않음 (아이콘 클릭 시에만 열림)' }
        ]
      }
    ]
  },
    ...
    subsections: [
      {
        ...
        items: [
          ...
          { id: '6.1.10', name: '위치 선택', method: '위치 드롭다운 확인', expected: '구매칸, 기부칸, 1~16번 칸 옵션 표시, 기본값 "구매칸"' },
      ...
      },
      {
        ...
        items: [
          ...
          { id: '6.4.6', name: '위치별로 보기', method: '"위치별로 보기" > "1번 칸" 선택', expected: '1번 칸에 있는 도서만 필터링' },
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        title: '8.2 도서 카드 공통',
        items: [
          { id: '8.2.1', name: 'NO IMAGE 표시', method: '표지 이미지 없는 도서 확인', expected: '동일 크기 박스에 회색 배경 + "NO IMAGE" 텍스트' },
          { id: '8.2.2', name: '저자/출판사 표시 형식', method: '도서 카드 저자/출판사 확인', expected: '"저자: [저자명]", "출판사: [출판사명]" 형식' },
          { id: '8.2.3', name: '긴 텍스트 처리', method: '저자/출판사명이 긴 도서 확인', expected: '한 줄에서 ... 으로 말줄임 처리' },
          { id: '8.2.4', name: '빈 정보 처리', method: '저자 또는 출판사 정보 없는 도서 확인', expected: '해당 필드 미표시, 전체 박스 높이 유지' },
          { id: '8.2.5', name: '위치 정보 표시', method: '도서 카드에서 위치 정보 확인', expected: '위치 아이콘 및 텍스트 표시 (예: "1번 칸", "구매칸")' }
        ]
      },
      {
        title: '8.3 로딩 상태',
        items: [
          { id: '8.3.1', name: '페이지 로딩', method: '페이지 새로고침', expected: '로딩 인디케이터 표시' },
          { id: '8.3.2', name: '도서 검색 로딩', method: '도서 검색 실행', expected: '검색 중 로딩 표시' },
          { id: '8.3.3', name: '버튼 로딩', method: '대여/반납 버튼 클릭', expected: '버튼에 로딩 스피너 표시' }
        ]
      },
      {
        title: '8.4 에러 상태',
        items: [
          { id: '8.4.1', name: '네트워크 오류', method: '오프라인 상태에서 작업', expected: '오류 메시지 표시' },
          { id: '8.4.2', name: '빈 목록', method: '검색 결과 없을 때', expected: '적절한 빈 상태 메시지' },
          { id: '8.4.3', name: '폼 유효성 오류', method: '잘못된 입력 제출', expected: '필드별 오류 메시지 표시' }
        ]
      }
    ]
  },
